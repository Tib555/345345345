document.addEventListener('DOMContentLoaded', () => {
    ...
    // Drag-to-scroll (прокрутка перетаскиванием мыши) для длинных формул
    let isDown = false;
    let startX;
    let scrollLeft;
    ...
    document.addEventListener('mousedown', (e) => {
        // Ищем контейнер формулы при клике
        const mjxContainer = e.target.closest('mjx-container[display="true"]');
        if (!mjxContainer) return;
        
        // Если формула не требует прокрутки (помещается на экран), не активируем drag
        if (mjxContainer.scrollWidth <= mjxContainer.clientWidth) return;

        isDown = true;
        activeSlider = mjxContainer;
