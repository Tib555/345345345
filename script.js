document.addEventListener('DOMContentLoaded', () => {
    ...
    // Улучшенный Drag-to-scroll (прокрутка перетаскиванием мыши) для длинных формул
    let isDown = false;
    let startX;
    let scrollLeft;
    ...
    document.addEventListener('mousedown', (e) => {
        let target = e.target;
        // Поднимаемся по DOM-дереву, чтобы найти обертку
        while (target && target !== document) {
            if (target.classList && target.classList.contains('math-scroll')) {
                break;
            }
        ...
        // Если формула не требует прокрутки (с небольшим запасом), не активируем drag
        if (scrollContainer.scrollWidth <= scrollContainer.clientWidth + 2) return;

        isDown = true;
        activeSlider = scrollContainer;
